...
res_sa20 = feols( log(remuneracao_media_real)~ sunab(ano_conclusao, event_time) | cpf + ano_observacao, data=df_1)
summary(res_sa20)
iplot(res_sa20)
...
df_1 = df %>%
  filter(ano_observacao >= 2009 & ano_observacao <= 2022)
...
df_1 = df %>%
  filter(ano_observacao >= 2010 & ano_observacao <= 2022)
summary(df_1$ano_observacao)
# First, create the 'event_time' variable as before
df_1 <- df_1 %>%
  group_by(cpf) %>%
...
df_1 = df %>%
  filter(ano_observacao >= 2011 & ano_observacao <= 2022)
summary(df_1$ano_observacao)
# First, create the 'event_time' variable as before
df_1 <- df_1 %>%
  group_by(cpf) %>%
